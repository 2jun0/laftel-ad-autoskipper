...
function checkAndMuteAdVideos(mute) {
  document.querySelectorAll(AD_VIDEO_SELECTOR_LIST.join(','))
    .forEach(video => { 
      mute ? video.volume = 0 : video.volume = 1;
      video.playsinline = "";
    });
}
...
if (/.*:\/\/.*laftel.net\/.*/.test(document.referrer || document.URL) && window.location.hostname == 'laftel.net') {
